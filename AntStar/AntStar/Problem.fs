...
let pickBox ((goalPos, gt): Pos * Goal) (prevH: Map<Pos*Pos,int>) (boxTypeToId: Map<ObjType, Set<Guid>>) (grid: Grid) : (Box * Pos) * Pos list = 
  match graphSearch (BoxPointerProblem (grid, goalPos, true, prevH, boxTypeToId.[gt] |> Set.map (fun id -> grid.boxPos.[id]))) with
  | Some [] ->
...
let pickAgent ((boxPos, boxColor): Pos * Color) (prevH: Map<Pos*Pos,int>) (agentColorToId: Map<Color,Set<AgentIdx>>) (grid: Grid): (Agent * Pos) * Pos list =
  match graphSearch (AgentPointerProblem (grid, boxPos, true, prevH, agentColorToId.[boxColor] |> Set.map (fun id -> grid.agentPos.[id]))) with
  | Some s -> 
    let state = s.Head.state
...
let rec solveObstacle prevH agentColorToId (reservedPath: Set<Pos>) (pos: Pos) (state: Grid): ActionList * Grid = 
  ...
  let freePositions = Set.difference free reservedPath
...
let solveGoal (goalPos, goal) prevH boxTypeToId agentColorToId grid : ActionList * Grid = 
  let box, agent, (grid', actions) = 
    createClearPath prevH boxTypeToId agentColorToId (goalPos, goal) grid   
