...
let findAdjecent (predicate: DynamicObject -> bool) (pos: Pos) (grid: Grid) : (Pos * DynamicObject) option = 
  ...
  ?|>
    function 
    | (p, Some o) -> (p, o) 
    | _ -> failwith "expected DynamicObject to be Some"
...
let root (p: IProblem<'s,'a>): Node<'s,'a> = 
  let s = p.Initial
  { state  = s;
    parent = None;
...
type SearchQueue<'s,'a when 's: comparison> = 
  ...
  member pq.Pop = 
    let el, q' = PriorityQueue.pop pq.q
    el, {pq with q = q'; m = pq.m.Remove el.state}
  member pq.Insert el = 
    {pq with 
      q = PriorityQueue.insert el pq.q;
...
[<AbstractClass>]
type IGridProblem<'a> () = inherit IProblem<Grid,'a>()
...
type PointerProblem (grid: Grid, startPos: Pos, goalTest: Pos -> Grid -> Boolean) = 
  inherit IGridProblem<Action []>()
  override p.Initial = {grid with searchPoint = Some startPos }
  override p.Actions s = 
    s
    |> Grid.validMovePointer 
    |> List.map (fun (a,g) -> (Array.create 1 a, g))
  override p.GoalTest s = 
    match s.searchPoint with 
...
let euclideanDistance (p1: Pos) (p2: Pos) = 
  ...
  Math.Sqrt (e1 + e2) |> int
let getEstimatedDistance grid endPos prevH currentCost =
  match Map.tryFind (grid.searchPoint.Value,endPos) prevH with
  | Some v -> v + currentCost
  | None -> Cost.MaxValue
...
type PosPointerProblem (grid, startPos, endPos, prevH) =
  ...
  override p.ChildNode n a s = 
    ...
    {child with value = getEstimatedDistance child.state endPos prevH child.cost}

let boxGoalTest goal pointer s = s.dynamicGrid.TryFind pointer |> Option.exists (isBoxOfType goal)

type ObjectPointerProblem(grid, startPos, addCost: bool, prevH: Map<Pos*Pos,int>, objectPositions: Set<Pos>) =
  inherit PointerProblem(grid, startPos, fun p -> fun _ -> objectPositions |> Set.contains p)
  ...
  override p.ChildNode n a s = 
    ...
    let h = 
      objectPositions
      |> Set.map (fun p -> getEstimatedDistance n'.state p prevH cost')
      |> Set.minElement
    {n' with value = h; cost = n'.cost;}

type BoxPointerProblem (grid, startPos, addCost: bool, prevH: Map<Pos*Pos,int>, boxPositions: Set<Pos>) =
  inherit ObjectPointerProblem(grid, startPos, addCost, prevH, boxPositions)

  
let agentGoalTest color pointer s = Option.exists (isAgentOfColor color) (s.dynamicGrid.TryFind pointer)
type AgentPointerProblem (grid, startPos, addCost: bool, prevH: Map<Pos*Pos,int>, agentPositions: Set<Pos>) =
  inherit ObjectPointerProblem(grid, startPos, addCost, prevH, agentPositions)
...
let graphSearch<'a> (p: IGridProblem<'a>) = 
...
let getBoxPositions (boxTypeToId: Map<ObjType,Set<Guid>>) objtype grid =
  boxTypeToId.[objtype] 
  |> Set.map (fun id -> grid.boxPos.[id])

let getAgentPositions (agentColorToId: Map<Color,Set<AgentIdx>>) color grid =
  agentColorToId.[color] 
  |> Set.map (fun id -> grid.agentPos.[id])
...
let rec orderGoals' (grid: Grid) (prevH: Map<Pos * Pos, int>) (isMA: bool) (boxTypeToId: Map<ObjType,Set<Guid>>) (agentColorToId: Map<Color,Set<AgentIdx>>) (orderedGoals : List<Pos * Goal>) (unsolvedGoals : Set<Pos * Goal>) = 
  ...
  let isSolvableGoal ((goalPos, gt): Pos * Goal) = 
    ...
    match BoxPointerProblem (grid', goalPos, false, prevH, getBoxPositions boxTypeToId gt grid') |> graphSearch with
    | Some n -> 
      if isMA |> not then true else
        ...
        let boxColor =
          ...
          | Box(_,_,c) -> Some c
          | _ -> None
        match AgentPointerProblem (grid', boxPos, false, prevH, getAgentPositions agentColorToId boxColor.Value grid') |> graphSearch with
        | Some _ -> true
        | None -> false  
    | None -> false  
  ...
  if unsolvedGoals.IsEmpty
    then orderedGoals
      ...
      match unsolvedGoals |> Set.toList |> List.tryFind isSolvableGoal with
      | Some g -> 
